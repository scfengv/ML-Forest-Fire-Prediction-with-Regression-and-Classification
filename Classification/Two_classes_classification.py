...
class FireModel:
    ...
    def train_and_evaluate(self, model):
        ...
        y_pred_score = model.predict_proba(self.x_test)[:, 1]

        print(classification_report(self.y_test, y_pred, target_names=["no fire", "fire"]))

        fpr, tpr, _ = roc_curve(self.y_test, y_pred_score, pos_label=model.classes_[1])
        roc_auc = auc(fpr, tpr)
        ...
        plt.savefig(f"result/Two_classes_{model.__class__.__name__}\nOne vs Rest ROC curve", dpi = 300)
        plt.show()
...
def main():
    ...
    fire_model = FireModel(x_train, y_train, x_test, y_test)
    ...
    x_train, x_test, y_train, y_test = train_test_split(x_smote, y_smote, train_size = 0.8, stratify = y_smote)
    fire_model = FireModel(x_train, y_train, x_test, y_test)
